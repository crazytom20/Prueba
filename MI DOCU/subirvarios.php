...
if (isset($_POST['ok'])) {
    foreach ($_FILES['archivo']['name'] as $i => $nombre) {   
        ...
        // Extrae su tipo MIME
        $tipo_mime = $_FILES['archivo']['type'][$i];
        
        // Extrae su tamaño
        $tamanio = $_FILES['archivo']['size'][$i];
        
        // Extrae su ubicación del archivo temporal
        $archivo_temporal = $_FILES['archivo']['tmp_name'][$i];
        
        // Extrae el codigo de error
        $codigo_error = $_FILES['archivo']['error'][$i];
        ...
        switch ($codigo_error) {
            ...
            // Copia el archivo temporal (probar el resultado)
                ...
                if (@move_uploaded_file($archivo_temporal,$destino)) {
                    ...
                    if ($query) {
                                echo $mensaje  =$nombre." subido correctamente". " ".$time.'</br>';
                             }
...
function uploadFile(){
	...
	ajax.open("POST", "subirvarios.php");
	ajax.send(formdata);
}
...
<div class="content-page-container full-reset custom-scroll-containers" >
        ...
        <div class="container">
            ...
            <?php echo $mensaje; ?>
            <h1 class="all-tittles">SUBIR VARIOS ARCHIVOS</h1>
            </div>
        </div>
		...
		<div class="row mt-3">
            ...
            <form action="" method="post" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="MAX_FILE_SIZE" value="300000000" />
                        ...
                        <input size="300000000" type="file"  id="idNombreT" name="archivo[]" id="archivo" class="form-control-file" multiple=""> 
                        </div>    
                </div>
		                       
                    <div class="form-group col-md-12">
                    <input type="submit" id="idDescripcionT" class="btn btn-primary" name="ok"  value="Enviar" onclick="uploadFile()"><br><br>
